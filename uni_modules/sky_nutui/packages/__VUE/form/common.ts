import { getPropByPath, isFunction, isObject, isPromise } from '@/uni_modules/sky_nutui/packages/utils/util';
...
export const component = {
  ...
  setup(props: any, { emit, slots }: any) {
    ...
    const slotNodes = ref([])
    provide('addNode', (item)=>{
        slotNodes.value.push(item)
        //slots.default = ()=>slotNodes.value
    });
    ...
    const validate = (customProp: string = '') => {
      return new Promise((resolve, reject) => {
        ...
        let task = findFormItem(slotNodes.value);
