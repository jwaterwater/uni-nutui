<template>
  <div class="demo">
    ...
    <nut-popup position="top" :popStyle="{ height: '20%' }" v-model:visible="state.showTop"></nut-popup>
    <nut-cell :title="translate('cell3')" is-link @click="state.showBottom = true"></nut-cell>
    <nut-popup position="bottom" :popStyle="{ height: '20%' }" v-model:visible="state.showBottom"></nut-popup>
    <nut-cell :title="translate('cell4')" is-link @click="state.showLeft = true"></nut-cell>
    <nut-popup position="left"  :popStyle="{ width: '20%', height: '100%' }" v-model:visible="state.showLeft"></nut-popup>
    <nut-cell :title="translate('cell5')" is-link @click="state.showRight = true"></nut-cell>
    <nut-popup position="right" :popStyle="{ width: '20%', height: '100%' }" v-model:visible="state.showRight"></nut-popup>
    <h2 class="h2">{{ translate('close') }}</h2>
    <nut-cell :title="translate('close')" is-link @click="state.showIcon = true"></nut-cell>
    <nut-popup position="bottom" closeable :popStyle="{ height: '20%' }" v-model:visible="state.showIcon"></nut-popup>
    ...
    ></nut-popup>
    <h2 class="h2">{{ translate('teleport') }}</h2>
    <nut-cell :title="translate('teleport')" is-link @click="state.showTeleport = true"></nut-cell>
    <nut-popup :popStyle="{ padding: '30px 50px' }" teleport="#app" v-model:visible="state.showTeleport">app</nut-popup>
    <h2 class="h2">{{ translate('muti') }}</h2>
    <nut-cell :title="translate('muti')" is-link @click="state.showPop1 = true"></nut-cell>
    <nut-popup :popStyle="{ padding: '30px 50px' }" v-model:visible="state.showPop1">
      <div @click="state.showPop2 = true">{{ translate('click') }}</div>
    </nut-popup>
    <nut-popup :popStyle="{ padding: '30px 50px' }" v-model:visible="state.showPop2">{{ translate('text') }}</nut-popup>
  </div>
</template>

<script lang="ts">
import { reactive } from 'vue';
import { createComponent } from '@/uni_modules/sky_nutui/packages/utils/create';
const { createDemo, translate } = createComponent('popup');
import { useTranslate } from '@/uni_modules/sky_nutui/sites/assets/util/useTranslate';
const initTranslate = () =>
  useTranslate({
    'zh-CN': {
      basic: '基本用法',
