<template>
  <nut-tabs autoHeight popClass="nut-cascader" v-model="tabsCursor" @click="handleTabClick" title-scroll>
    <template v-if="!initLoading && panes.length">
      <nut-tabpane v-for="(pane, index) in panes" 
      :paneKey="index"
      :title="formatTabTitle(pane)" :key="index">
        <view role="menu" class="nut-cascader-pane">
          <template v-for="node in pane.nodes" :key="node.value">
            ...
            >
              ...
              <nut-icon v-if="node.loading" popClass="nut-cascader-item__icon-loading" name="loading" />
              <nut-icon v-else popClass="nut-cascader-item__icon-check" name="checklist" />
            </view>
          </template>
        </view>
      </nut-tabpane>
